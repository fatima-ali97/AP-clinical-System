...
$projectDir    = Join-Path $scriptDir "AP-clinical-system.API"
$entitiesDir   = Join-Path $projectDir "Models\Entities"
$contextFile   = Join-Path $projectDir "Models\sql_Context\AP_Context.cs"
$migrationsDir = Join-Path $projectDir "Models\sql_Context\Migrations"
...
$buildOutput = & dotnet build "$projectDir" --nologo -v q 2>&1
...
$pendingCheck = & dotnet ef migrations has-pending-model-changes --project "$projectDir" --output-dir $migOutputDir 2>&1
$hasPending = $false
...
if ($hasPending) {
    ...
    $migOutput = & dotnet ef migrations add $Name --project "$projectDir" --output-dir $migOutputDir 2>&1
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        if ($latestMigration) {
            ...
            if ($migContent -match '(?s)void Up\(.*?\)\s*\{(\s*)\}') {
                ...
                & dotnet ef migrations remove --project "$projectDir" --output-dir $migOutputDir --force 2>&1 | Out-Null
            } else {
...
$pendingMigrations = & dotnet ef migrations list --project "$projectDir" --output-dir $migOutputDir --no-connect 2>&1 |
...
if (-not $Force) {
    ...
    $config   = Get-Content (Join-Path $projectDir "appsettings.json") -Raw | ConvertFrom-Json
